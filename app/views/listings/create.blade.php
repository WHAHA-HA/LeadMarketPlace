...
<link rel="stylesheet" type="text/css" href="{{asset('css/vendors/jquery.datetimepicker.css')}}"/ >
 <style>
        ...
        .wizard .nav-tabs li a .number{background-color: #EEE;display: inline-block;text-align: center !important;font-size: 16px;font-weight: 300;padding: 9px 15px 13px 15px;margin-right: 10px;height: 40px;width: 40px;-webkit-border-radius: 50% !important;-moz-border-radius: 50% !important;border-radius: 50% !important;}
        ...
        .wizard .nav-tabs > li > a{background-color:transparent;font-size:13px;}
        .error{color:red;}
</style>
@stop

@section('content')
    {{Former::vertical_open()->populate($listing?$listing:array())->action(route('listings.store'))->role('form')->id('apptshareform')}}
...
$('#apptsharewizard').bootstrapWizard({
	...
	onTabShow: function(tab, navigation, index) 
	{
		...
		if($current >= $total) {
            ...
            $('#apptsharewizard').find('.paging .publish').show();
            $('#apptsharewizard').find('.paging .publish').removeClass('disabled');
        } else {
            $('#apptsharewizard').find('.paging .next').show();
            $('#apptsharewizard').find('.paging .publish').hide();			
		}
...
$('[name="for_points"]').change(function(){
	option = $(this).val();
